...
class Attention(nn.Module):
    ...
    def forward(self, h, enc_out):
        ...
        att = self.softmax(qk)
        out = torch.einsum('bij,bjk->bik',att,v) #[8,2,256]
...
class ATT_Decoder(nn.Module):
    def __init__(self, enc_hidden_size, dec_hidden_size, num_layers, attention, embedding_dim, dropout_rate,dynamicDe_num,bidirectional=False):
        ...
        self.fc_sentinel_1 = nn.Linear(embedding_dim, embedding_dim)
        self.fc_sentinel_2 = nn.Linear(embedding_dim, embedding_dim)
        '''
    ...
    def forward(self, dec_input, dyanmic_dec_input,h, c, enc_output):
        ...
        #embedded = dec_input
        #print(dec_input.shape, dyanmic_dec_input.shape)
        ...
        dec_output, (dec_h, dec_c) = self.lstm(embedded, (h, c))
        g_t = torch.sigmoid(self.fc_sentinel_1(embedded) + self.fc_sentinel_2(h[-1].unsqueeze(1)))
        s_t = g_t*torch.tanh(dec_c[-1].unsqueeze(1))
        att = self.attention(dec_h[-1:], torch.cat((enc_output, s_t.transpose(0,1))))
