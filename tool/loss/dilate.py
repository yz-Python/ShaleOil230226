...
class dilate_loss(torch.nn.Module):
    def __init__(self, alpha=0.5, gamma=0.001):
        super(dilate_loss, self).__init__()
        self.alpha = alpha
        self.gamma = gamma
...
if __name__ =='__main__':
    batch_size, len_x, dims=16,3,1
    x = torch.rand((batch_size, len_x, dims), requires_grad=True)
    y = torch.rand((batch_size, len_x, dims))
    criterion = dilate_loss()#(gamma=1.0, normalize=True) # just like nn.MSELoss()
    loss, loss_shape, loss_temporal = criterion(x, y)
    print(loss)
    #loss = sum(loss)
    loss.backward()