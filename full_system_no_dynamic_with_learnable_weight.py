...
from model.full_model_no_dynamic_with_learnable_weight import Net
...
def train(config, logger, train_and_valid_data):
    ...
    encoder_train, decoder_train, label_train= torch.from_numpy(encoder_train).float(), torch.from_numpy(decoder_train).float(), torch.from_numpy(label_train).float()   # 先转为Tensor
    print(encoder_train.shape,decoder_train.shape,label_train.shape)
    train_loader = DataLoader(TensorDataset(encoder_train, decoder_train,label_train,full_s_h_train,full_s_n_train), batch_size=config.batch_size)    # DataLoader可自动生成可训练的batch数据

    encoder_valid, decoder_valid, label_valid= torch.from_numpy(encoder_valid).float(), torch.from_numpy(decoder_valid).float(), torch.from_numpy(label_valid).float()
    valid_loader = DataLoader(TensorDataset(encoder_valid, decoder_valid, label_valid,full_s_h_valid,full_s_n_valid), batch_size=config.batch_size)
    ...
    for epoch in range(config.epoch):
        ...
        for i, _data in loop:
                ...
                _encoder_train, _decoder_train, label_train,human,nature= _data[0].to(device),_data[1].to(device),_data[2].to(device),_data[3].to(device),_data[4].to(device)
        ...
        for i, _data in loopv:
            ...
            encoder_valid, decoder_valid, label_valid ,human,nature= _data[0].to(device),_data[1].to(device),_data[2].to(device),_data[3].to(device),_data[4].to(device)
...
if __name__=="__main__":
    ...
    parser.add_argument("--name", default='A1,B1,B124,B590_1,B590_2,A26', type=str, help="填写训练预设名称")#可以填写'All' 或者'A1,B1,B590_1,B124,A26'这种形式
    ...
    end_sign = '_RMLSE_learnWeight'+str(con.batch_size)
